...
`나라장터 공고 컬렉터`는 이런 반복 업무를 줄이기 위해 만든 초보자 친화형 공고 수집/정리 도구입니다.
...
* 샘플 공고 데이터 실행 지원
* 공고번호, 공고명, 기관명, 예산, 마감일, 업종제한 정리
* 나라장터 API 업무구분을 기준으로 공사·물품·용역·내자 기본 구분
* 실무 검토용 CSV 파일 생성
* 실무 검토용 Excel 파일 생성
* 로컬 웹 UI에서 검색 프리셋, 유형 필터, 정렬, 선택 내보내기, 공고 상세 확인 지원
...
로컬 웹 UI는 브라우저에서 공고를 검토하는 작업 공간입니다.

지원하는 작업은 다음과 같습니다.

* `최근 7일`, `이번 달`, `행정복지센터` 검색 조건 프리셋 적용
* 공사·물품·용역·내자 유형 필터
* 공고번호순, 마감일 빠른순, 예산 높은순 정렬
* 선택한 공고만 CSV/Excel로 내보내기
* 공고 상세 패널에서 공고번호, 기관명, 마감일, 예산 확인
* `공고문 페이지` 원문 열기와 `공고문 보기` Synap 문서뷰어 열기
...
로컬 웹 UI에서 각 공고의 `공고문` 버튼을 누르면 나라장터 공고문을 새 탭으로 엽니다.

나라장터 API 응답에 `stdNtceDocViewUrl`, `ntceSpecDocViewUrl1~10`, `synapDocViewUrl` 같은 G2B Synap 문서뷰어 URL이 포함되어 있으면 그 URL을 그대로 엽니다.
...
PR에서는 아래 검증이 자동으로 실행됩니다.

* TypeScript 타입 검사
* Vitest 테스트
* Vite 웹 UI 빌드

릴리즈는 `v0.4.0` 같은 태그를 push하면 GitHub Actions의 `Release` 워크플로가 실행됩니다.
