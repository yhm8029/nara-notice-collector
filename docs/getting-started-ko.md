...
나라장터 API 응답에 G2B Synap 문서뷰어 URL이 있으면 그 URL을 바로 열고, 없으면 나라장터 첨부파일 URL을 Synap viewer URL로 변환해서 엽니다.
변환에 실패하면 원문 링크나 다운로드 링크로 이동하지 않고 오류 메시지를 표시합니다.

웹 화면에서는 `최근 7일`, `이번 달`, `행정복지센터` 프리셋으로 검색 조건을 빠르게 바꿀 수 있습니다.
공고 목록은 유형별로 필터링하고 마감일 또는 예산 기준으로 정렬할 수 있습니다.
필요한 공고를 체크한 뒤 CSV 또는 Excel 버튼을 누르면 선택한 공고만 내보냅니다.
행의 `상세` 버튼을 누르면 오른쪽 상세 패널에서 기관명, 마감일, 예산을 확인할 수 있습니다.
...
설정하지 않아도 나라장터 첨부 URL은 G2B Synap viewer URL로 변환해서 엽니다.
